...
void wd()
{
	...
	//char tmp[128];
	//gcvt(temp,4,tmp);
	extern int fBodyTempture;
	//strrpl(yi[2],16,"TEM %f\n", fBodyTempture);
	//strrpl(yi[2],yi[2],sizeof(fBodyTempture),"",fBodyTempture);
	//char string[16] = {};
	//itoa(fBodyTempture,string,10);
	char s[16] = "";
	sprintf(s, "%d", fBodyTempture);
	memcpy(yi[2], s, strlen(s) + 1);
	if (fBodyTempture < 37) {
		char a[20] = "ALLOW";
		memcpy(yi[3], a, strlen(a) + 1);
	}
	else {
		char b[20] = " NOT ALLOW";
		memcpy(yi[3], b, strlen(b) + 1);
	}
}
int screen(void)
{
